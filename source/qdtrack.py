...
class QDTrack(TwoStageDetector):
    ...
    def get_embeddings(self, img, img_metas, rescale=False):
        return_loss=False
        img = img[0]
        ...
        if track_feats is not None:
            bboxes, labels, ids, embeds = self.tracker.get_match_embed(
                bboxes=det_bboxes,
                labels=det_labels,
                track_feats=track_feats,
                frame_id=frame_id)
        ...
        return dict(bbox_results=bbox_result, track_results=track_result), dict(box=det_bboxes, det_label=det_labels, track_feat=track_feats, ids=ids, embeds=embeds)
